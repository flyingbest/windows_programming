...
CreateWindow() 함수를 사용하여 윈도우를 생성하면 윈도우가 설정한 내용이 적용된 윈도우가 생성된다. 하지만 윈도우가 생성만 될 뿐 화면에 출력되지는 않기 때문에 컴퓨터 사용자가 눈으로 확인할 수 없는 상태로 존재. 즉, 메모리 상에만 존재하는 윈도우가 생성된 것. 만들어지 윈도우를 화면에 나타내기 위해서는 ShowWindow() 함수를 사용.
```
ShowWindow(hWnd, nDmdShow)
```
...
|SW_SHOW|윈도우의 형태대로 화면에 출력한다.|
|SW_MINIMIZE|윈도우를 최소화 상태로 바꾼다.|

> 메시지 루프

윈도우 운영체제는 메시지를 받아서 처리하는 시스템(Message Driven System)이다. 즉, 프로그램이 미리 정해져 있는 각본에 따라서 처리되는 것이 아니라 사용자의 조작에 따라 그에 맞는 내용들을 순서대로 처리하는 방식의 시스템.

메시지란 미리 정의되어 있는 특정 동작들에 대한 값을 의미. 키보드 입력, 마우스 움직임 등이 대표적인 메시지라고 할 수 있다.
메시지가 생성되면 생성된 메시지는 메시지 큐(Message Queue)라는 장소에 차곡차곡 쌓이게 된다. 메시지 루프는 메시지 큐에 쌓여있는 메시지들을 읽어서 처리하는 루프를 의미하며, 다음과 같은 형식으로 작성하여 사용.
```
while (GetMassage (&Message, 0, 0, 0) ) {
	TranslateMessage(&Message);
	DispatchMessage(&Message);
}
```

메시지 루프는 다음과 같은 순서를 반복해서 실해되도록 구성
 1. 메시지 큐에서 메시지를 가져온다.
 2. 메시지를 가공한다.
 3. 가공된 메시지를 메시지 처리 함수로 전달한다.

```
BOOL GetMessage(LPMSG lpMsg, HWND hWnd, UNIT wMsgFilterMin, UNIT wMsgFilterMax);
```
GetMessage() 함수는 메시지 큐에서 메시지를 읽어오는 역할을 담당하는 함수. 읽어온 메시지가 WM_QUIT(윈도우 종료 메시지)인 경우에만 리턴 값이 FALSE로 전달되며, 나머지 메시지들인 경우에는 TRUE로 전달. 따라서 메시지 루프는 윈도우를 종료하라는 메시지가 전달될 때까지 계속해서 처리되도록 구성해줘야 함.

LPMSG lpMsg : 읽어온 메시지를 저장할 MSG 구조체 포인터를 지정하기 위한 인수. 메시지 큐에서 읽어온 메시지의 내용이 저장됨.
